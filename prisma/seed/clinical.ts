import {
  PrismaClient, ConsultaEstado, DienteSuperficie,
  TreatmentStepStatus, ClinicoArchivoTipo,
  DiagnosisStatus, AllergySeverity, PerioSite, PerioBleeding,
  AdjuntoTipo
} from "@prisma/client";
...
export async function createPlanConSteps(
  prisma: PrismaClient,
  params: {
    pacienteId: number; createdByUserId: number;
    steps: Array<{
      code?: string; serviceType?: string;
      toothNumber?: number | null; toothSurface?: DienteSuperficie | null;
      estimatedDurationMin?: number | null; estimatedCostCents?: number | null;
...
export async function ensureConsultaParaCita(
  prisma: PrismaClient,
  params: {
    citaId: number; performedByProfessionalId: number; createdByUserId: number;
    status?: ConsultaEstado; reason?: string | null; diagnosis?: string | null; clinicalNotes?: string | null;
  }
) {
...
export async function addLineaProcedimiento(
  ...
  params: {
    ...
    code?: string; serviceType?: string;
    toothNumber?: number | null; toothSurface?: DienteSuperficie | null;
    quantity?: number; unitPriceCents?: number | null; totalCents?: number | null;
    treatmentStepId?: number | null; resultNotes?: string | null;
  }
) {
  let procedureId: number | undefined = undefined;
  if (params.code) {
