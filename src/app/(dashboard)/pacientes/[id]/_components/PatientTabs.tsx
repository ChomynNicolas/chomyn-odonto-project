...
export function PatientTabs({ patientId, currentRole }: PatientTabsProps) {
  ...
  const activeTab = useMemo(() => {
    ...
    // Default: anamnesis for clinical roles, administrative for RECEP
    return currentRole === "RECEP" ? "administrative" : "anamnesis"
  }, [urlTab, visibleTabs, currentRole])

  // Handle tab change - update URL without page reload
  const handleTabChange = (value: string) => {
    const params = new URLSearchParams(searchParams.toString())
    const defaultTab = currentRole === "RECEP" ? "administrative" : "anamnesis"
