// src/app/(dashboard)/reportes/diagnosticos-resueltos/page.tsx
"use client"

/**
 * Resolved Diagnoses Report Page
 * Lists resolved diagnoses with resolution time metrics and therapeutic success audit.
 */

import { useState, useCallback, useEffect } from "react"
import { ReportShell, ReportKpiCards, ReportTable, ReportFiltersForm, ReportExportButtons } from "@/components/reportes"
import { diagnosticosResueltosFiltersSchema } from "@/lib/validation/reportes"
import {
  REPORT_CONFIGS,
  type DiagnosticosResueltosFilters,
  type DiagnosticosResueltosResponse,
  type DiagnosticoResueltoRow,
} from "@/types/reportes"
import { toast } from "sonner"

/** Table columns for resolved diagnoses */
const COLUMNS = [
  ...
  { key: "fechaRegistro", label: "Fecha Registro", format: "date" as const },
  { key: "fechaResolucion", label: "Fecha Resolución", format: "date" as const },
  { key: "tiempoResolucionDias", label: "Tiempo Resolución (días)", align: "center" as const },
  { key: "createdBy.nombreApellido", label: "Profesional" },
  { key: "consulta.fecha", label: "Consulta", format: "date" as const },
]

export default function DiagnosticosResueltosReportPage() {
  const [isLoading, setIsLoading] = useState(false)
  const [data, setData] = useState<DiagnosticosResueltosResponse | null>(null)
  const [error, setError] = useState<string | null>(null)
  const [currentFilters, setCurrentFilters] = useState<DiagnosticosResueltosFilters | null>(null)
  ...
  const config = REPORT_CONFIGS["diagnosticos-resueltos"]
  ...
  const fetchReport = useCallback(async (filters: DiagnosticosResueltosFilters) => {
    setIsLoading(true)
    setError(null)
    setCurrentFilters(filters)

    try {
      const response = await fetch("/api/reportes/diagnosticos-resueltos", {
        method: "POST",
  ...
  const renderCell = useCallback((column: { key: string; label: string }, row: DiagnosticoResueltoRow) => {
    ...
    if (column.key === "fechaRegistro") {
      return new Date(row.fechaRegistro).toLocaleDateString("es-PY")
    }
    if (column.key === "fechaResolucion") {
      return new Date(row.fechaResolucion).toLocaleDateString("es-PY")
    }
    if (column.key === "tiempoResolucionDias") {
      const dias = row.tiempoResolucionDias
      const isRapido = dias < 30
      const isNormal = dias >= 30 && dias <= 90
      const isLento = dias > 90
      return (
        <span
          className={
            isRapido
  ...
  return (
    <ReportShell
      ...
      filters={
        <ReportFiltersForm
          schema={diagnosticosResueltosFiltersSchema}
    ...
    >
      ...
      {data ? (
        <ReportTable
          ...
          isLoading={isLoading}
          emptyMessage="No se encontraron diagnósticos resueltos con los filtros seleccionados"
