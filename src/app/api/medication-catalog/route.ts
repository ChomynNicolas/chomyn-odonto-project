// src/app/api/medication-catalog/route.ts
/**
 * GET /api/medication-catalog - Lista medication catalogs con filtros
 * POST /api/medication-catalog - Crea un nuevo medication catalog
 */

import { type NextRequest, NextResponse } from "next/server"
import { requireSessionWithRoles } from "../_lib/auth"
import { MedicationCatalogListQuerySchema, MedicationCatalogCreateBodySchema } from "./_schemas"
import { listMedicationCatalogs, createMedicationCatalog } from "./_service"
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const parsed = MedicationCatalogListQuerySchema.safeParse(query)
    ...
    const result = await listMedicationCatalogs(parsed.data)
    ...
    if (auth.session?.user?.id && auth.role === "ADMIN") {
      await safeAuditWrite({
        ...
        action: AuditAction.MEDICATION_CATALOG_LIST_VIEW,
    ...
    console.error("GET /api/medication-catalog error:", code || errorMessage)
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsed = MedicationCatalogCreateBodySchema.safeParse(body)
    ...
    const medicationCatalog = await createMedicationCatalog(
      parsed.data,
      actorId,
      req.headers,
    ...
    return NextResponse.json({ ok: true, data: medicationCatalog }, { status: 201 })
  } catch (e: unknown) {
    const errorMessage = e instanceof Error ? e.message : String(e)
    ...
    console.error("POST /api/medication-catalog error:", errorMessage)
    return NextResponse.json({ ok: false, error: "INTERNAL_ERROR" }, { status: 500 })
  }
}
