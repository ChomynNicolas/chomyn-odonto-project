...
import PatientQuickCreateModal from "@/components/pacientes/PatientQuickCreateModal";
import {
  RelacionPacienteEnum,
  type RelacionPacienteEnum as RelacionEnumSchema,
...
async function resolvePersonaIdFromPacienteId(idPaciente: number): Promise<number> {
  const res = await fetch(`/api/pacientes/${idPaciente}`, { headers: { Accept: "application/json" }, cache: "no-store" });
  const data = await res.json();
...
function PersonaBadge({ p }: { p: PersonaListItemDTO }) {
  ...
  return (
    <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
      ...
      <div className="text-xs text-muted-foreground">{doc}{ruc}</div>
    </div>
  );
}
...
export default function ResponsablePagoSelector({
  ...
  const { data, isFetching, isError, error: qError } = useQuery({
        ...
        <>
          <div className="relative">
            ...
            {openList && enabled && (
              ...
              >
                {error && (
                  <div className="px-2 py-3 text-sm text-destructive">Error: {error}</div>
                )}
                {!error && empty && (
                  ...
                  items.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground">{doc}{ruc}{contact ? ` • ${contact}` : ""}</span>
                      </button>
                    );
                  })}
                {data?.hasMore && (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div className="flex items-end gap-2">
              ...
              <Label htmlFor="rp-principal-2" className="text-sm">Principal</Label>
            </div>
