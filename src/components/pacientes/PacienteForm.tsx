...
import { pacienteSchema, generoEnum, tipoDocumentoEnum } from "./types"
...
export default function PacienteForm({ defaultValues, onSubmit, submitLabel = "Guardar" }: Props) {
  ...
  defaultValues: {
    nombreCompleto: "",
    genero: "NO_DECLARA",
    tipoDocumento: "CI",                // ⬅️ nuevo default
    fechaNacimiento: "",
    dni: "",
    ruc: "",
    telefono: "",
...
const todayYMD = React.useMemo(() => new Date().toISOString().slice(0, 10), [])
  ...
  try {
    const payload = {
      ...
      // ⬇️ si viene "" lo mandamos como undefined; si Zod lo coaccionó a Date, lo dejamos
      fechaNacimiento:
        values.fechaNacimiento instanceof Date ? values.fechaNacimiento : undefined,
    }
        ...
        <FormItem>
          ...
          <FormControl>
            <Input
              type="date"
              max={todayYMD}
              // Soportar que RHF tenga Date | "" | undefined
              value={
              ...
              onChange={(e) => field.onChange(e.target.value)}
              // Si quieres ocultar el ícono nativo (tu CSS ya lo prevé):
...
/>  
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          ...
          <FormDescription>
            Formato: 6–8 dígitos + guion + dígito verificador (ej: 5123456-7).
          </FormDescription>
          <FormMessage />
        </FormItem>
      )}
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          ...
          <FormControl>
            <Input placeholder="+59599123456" inputMode="tel" {...field} />
          </FormControl>
    ...
    />
          ...
          <section aria-labelledby="sec-clinicos" className="rounded-lg border border-border bg-card p-4 sm:p-6">
            <h3 id="sec-clinicos" className="mb-4 text-sm font-medium text-foreground">
              Datos clínicos
            </h3>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
          ...
          <section aria-labelledby="sec-adjuntos" className="rounded-lg border border-border bg-card p-4 sm:p-6">
            ...
            />
            <p className="mt-2 text-xs text-muted-foreground">
              Próximamente: Cloudinary/S3 con verificación y antivirus.
            </p>
          </section>
